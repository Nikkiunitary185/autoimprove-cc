...
Usage:
    python report.py --target /path/to/project [--log-path <path>]
"""
...
def resolve_target(target: str) -> tuple[Path, Path]:
    ...
    if p.is_file() and p.name == "CLAUDE.md":
        return p.parent, p
    if p.is_dir():
        return p, p / "CLAUDE.md"
    print(f"오류: 유효하지 않은 경로입니다: {target}", file=sys.stderr)
    sys.exit(1)


def find_history_log(autoimprove_dir: Path) -> Path | None:
    """히스토리 파일을 찾는다."""
    history_path = autoimprove_dir / "history.jsonl"
...
def render_histogram(history: list[dict], width: int = 40) -> str:
    ...
    for entry in history:
        ...
        if iteration == best_iteration and action != "initial":
            best_marker = f" {YELLOW}<-- best{RESET}"
...
def get_recent_commits(target_dir: Path, count: int = 5) -> list[str]:
    """최근 커밋 메시지를 반환한다."""
    try:
...
def generate_report(
    ...
    """텍스트 리포트를 생성한다."""
    if not history:
        return f"  {target_name}에 대한 히스토리가 없습니다."
    ...
    lines: list[str] = [
        f"\n{'=' * 60}",
        f"  {BOLD}AutoImprove Report: {target_name}{RESET}",
        f"{'=' * 60}",
        "",
        f"  초기 점수   : {initial_score:.0%}",
        f"  최종 점수   : {BOLD}{final_score:.0%}{RESET}",
        f"  개선 폭     : {'+' if final_score > initial_score else ''}"
        f"{final_score - initial_score:.0%}",
    ...
    if target_dir and target_dir.exists():
        recent = get_recent_commits(target_dir, count=5)
        if recent:
    ...
    if final_score >= 1.0:
        lines.append(f"  {GREEN}{BOLD}100% 달성! 완벽합니다!{RESET}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog="예시: python report.py --target /path/to/project",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--log-path",
        help="특정 로그 파일 경로 (기본: <target>/.autoimprove/history.jsonl)",
    )
    args = parser.parse_args()

    config = load_config()
    target_dir, _ = resolve_target(args.target)
    target_name = target_dir.name

    eval_dir_name: str = config.get("eval_dir", ".autoimprove")
    autoimprove_dir = target_dir / eval_dir_name

    if args.log_path:
        log_path: Path | None = Path(args.log_path).expanduser().resolve()
    else:
        log_path = find_history_log(autoimprove_dir)

    if not log_path or not log_path.exists():
        print(
            f"오류: {target_name}에 대한 로그를 찾을 수 없습니다.\n"
            f"검색 경로: {autoimprove_dir}",
            file=sys.stderr,
        )
        sys.exit(1)

    history = load_history(log_path)
    report = generate_report(target_name, history, target_dir)
    print(report)

    # 리포트 파일 저장
    autoimprove_dir.mkdir(parents=True, exist_ok=True)
    report_path = autoimprove_dir / "report.txt"
