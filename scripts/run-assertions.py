...
Usage:
    python run-assertions.py --target /path/to/project [--verbose]
    python run-assertions.py --target /path/to/project/CLAUDE.md [--verbose]
"""
...
def judge_assertion(
    ...
    *,
    ...
    prompt = (
        "You are a strict evaluator. Given the following CLAUDE.md content, "
...
def run_assertions(
    ...
    *,
    ...
    if not claude_md_path.exists():
        print(
            f"오류: CLAUDE.md를 찾을 수 없습니다: {claude_md_path}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    for test in eval_data["tests"]:
        ...
        for assertion in test["assertions"]:
            ...
            result, raw_output = judge_assertion(
                claude_md_content, assertion, llm_cmd, verbose=verbose
            )
...
def main() -> dict:
    parser = argparse.ArgumentParser(
        description="eval.json의 assertion을 CLAUDE.md에 대해 채점합니다.",
        epilog="예시: python run-assertions.py --target /path/to/project --verbose",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--eval-path", help="eval.json 경로 (기본: <target>/.autoimprove/eval.json)"
    )
    parser.add_argument(
    ...
    results = run_assertions(
        eval_data, claude_md_path, config, verbose=args.verbose
    )
    total_elapsed = time.monotonic() - start_time
